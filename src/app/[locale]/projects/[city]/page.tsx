...
export async function generateMetadata(
    ...
    const metatitle = unslugify(city) + " Egypt Real Estate Projects | Egypt Real Estate" ;
    const metadesc = unslugify(city) + " Real estate in Egypt. Properties for sale in Egypt.";
    return {
        title: metatitle,
...
export default async function CityPage({
    ...
    const data = await getLookupId(unslugify(city), 'city');

    return (
        <>
            <ProjectsPage category="city" page={currentPage} dataid={data} city={unslugify(city)} community={community} subcommunity={subcommunity} project={project} propertyname={propertynamefinal} isFeaturedProjectOnWeb={isFeaturedProjectOnWebfinal}/>
        </>
    );
}