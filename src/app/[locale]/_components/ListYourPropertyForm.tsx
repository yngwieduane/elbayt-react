...
const ListYourPropertyForm: React.FC<InquiryFormProps> = ({ hideFeedbackButton = false }) => {
  ...
  const onSubmit = async (data: FormData) => {
    sendGTMEvent({ event: 'List Your Property', value: '1' })
    ...
    const formDataToSend = {
        ...
        email: data.email,
        phone: data.phone,
        message: "List Your Property : "+data.message,
  ...
  return (
    <>
      <form onSubmit={handleSubmit(onSubmit)} className="w-full bg-white p-6 rounded-lg">
        ...
        {postId === "Success" && <div className="p-3 mb-3 rounded bg-ebGreen text-white">Form submitted successfully!</div>}
        {postId === "Error" && <div className="p-3 mb-3 rounded bg-red-500 text-white">Submission failed. Try again.</div>}
        <h2 className="text-lg mb-4" >Tell us about yourself</h2>
